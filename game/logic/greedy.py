...
class GreedyLogic(BaseLogic):
    ...
    def distance_matrix(self, goal: Position) -> list[list[int]]:
        ...
        is_vulnerable: list[list[bool]] = [[False for i in range(15)] for j in range(15)]
        for enemy in self.board.bots:
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    if (dx == 0 or dy == 0) and 0 <= enemy.position.x + dx < 15 and 0 <= enemy.position.y + dy < 15:
                        is_vulnerable[enemy.position.x + dx][enemy.position.y + dy] = True
        ...
        while not q.empty():
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    if ((dx == 0) ^ (dy == 0)) and 0 <= x + dx < 15 and 0 <= y + dy < 15:
                        if is_vulnerable[x + dx][y + dy]:
                            continue
    
                        is_first_teleporter = (Position(y + dy, x + dx) == first_teleporter)
                        is_second_teleporter = (Position(y + dy, x + dx) == second_teleporter)
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        if     (board_bot.properties.diamonds == board_bot.properties.inventory_size
                or distance(cur_pos, closest_diamond) + distance(closest_diamond, base) + RETREAT_DELAY > (board_bot.properties.milliseconds_left) / 1000.0
                or (board_bot.properties.diamonds == board_bot.properties.inventory_size - 1 and is_red[closest_diamond.x][closest_diamond.y])):
            goal = base
        else:
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if ((dx == 0) ^ (dy == 0)) and 0 <= cur_pos.x + dx < 15 and 0 <= cur_pos.y + dy < 15:
