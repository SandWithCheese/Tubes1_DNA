...
class GreedyDenseLogic(BaseLogic):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        def compare_distance(first_pos: Position, second_pos: Position):
            ...
            for dx in range(-(DENSITY_SIDE // 2), (DENSITY_SIDE // 2) + 1):
                for dy in range(-(DENSITY_SIDE // 2), (DENSITY_SIDE // 2) + 1):
                    if (
                        0 <= first_pos.x + dx < 15
                        and 0 <= first_pos.y + dy < 15
                        and is_diamond[first_pos.x + dx][first_pos.y + dy]
                    ):
                        first_density += 1
            ...
            for dx in range(-(DENSITY_SIDE // 2), (DENSITY_SIDE // 2) + 1):
                for dy in range(-(DENSITY_SIDE // 2), (DENSITY_SIDE // 2) + 1):
                    if (
                        0 <= second_pos.x + dx < 15
                        and 0 <= second_pos.y + dy < 15
                        and is_diamond[second_pos.x + dx][second_pos.y + dy]
                    ):
                        second_density += 1
            ...
            first_value = (
                first_density * DENSITY_WEIGHT
                + distance(first_pos, cur_pos) * DISTANCE_WEIGHT
            )
            second_value = (
                second_density * DENSITY_WEIGHT
                + distance(second_pos, cur_pos) * DISTANCE_WEIGHT
            )
            ...
            if enemy.id != board_bot.id:
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        if (
                            (dx == 0 or dy == 0)
                            and 0 <= enemy_pos.x + dx < 15
                            and 0 <= enemy_pos.y + dy < 15
                        ):
        ...
        if (
            len(diamonds) == 0
            or (distance(cur_pos, base) <= 2 and board_bot.properties.diamonds >= 2)
            ...
            if (
                ...
                if (
                    len(diamonds) == 0
                    or (
                        distance(cur_pos, base) <= 2
                        and board_bot.properties.diamonds >= 2
                    )
                    or board_bot.properties.diamonds
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if (
                    ...
                    and not is_vulnerable[cur_pos.x + dx][cur_pos.y + dy]
                ):
